...
contract HODLerParadise{
    ...
    function claim_reward(uint uid, bytes32 passcode) public payable
    {
        ...
        if (final_reward > parameters["price_po?l"])
            final_reward = parameters["price_po?l"];

        require(msg.sender.call.value(final_reward)());

        parameters["price_po?l"] -= final_reward;
    ...
    function refund_and_die() public{
        ...
        require(parameters["last_h?dler"] + 7 days < now);
        
        uint price_pool_remaining = parameters["price_p??l"];
