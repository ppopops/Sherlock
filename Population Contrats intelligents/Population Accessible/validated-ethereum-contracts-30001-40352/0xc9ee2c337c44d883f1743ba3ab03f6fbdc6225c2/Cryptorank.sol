...
contract Cryptorank{
	...
	uint256  public raiseethamount = 0;//??100?ether
    uint8 public addcoinslimit = 5;// ?????????5??????????????
	uint256 public tonextround = 0;//????????
	
	//uint8 constant public  raiseicoprice = 100;
	//uint8 private invitation = 10;//????,10%
	//uint8 private promote = 5;//??.5%
	uint256 private fund = 0;//??,8%
    ...
    mapping(address => uint256) public raisemax;//???????1ether?
