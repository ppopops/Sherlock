...
contract POCToken{
    ...
    uint public totalSupply = 1e28;//??100?

    uint public offline = 6e27;//??????60?
    uint private retention = 3e27;//??30?

    uint public airdrop = 1e27;//??10?
    uint public airdropLimit = 4e23;//????????????40?
    uint public fadd = 3e20;//?????300
    uint public fshare = 5e19;//???50
    ...
    function specialAddress(address addr) private pure returns(bool spe) {//??????POC???0????????1??????
