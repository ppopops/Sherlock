...
contract FoMo3Dlong is modularLong {
    ...
    mapping (address => uint256) public pIDxAddr_;          // ???? =? ??ID
    mapping (bytes32 => uint256) public pIDxName_;          // ?????=? ??ID
    mapping (uint256 => F3Ddatasets.Player) public plyr_;   // ??ID =? ????
    mapping (uint256 => mapping (uint256 => F3Ddatasets.PlayerRounds)) public plyrRnds_;    // ??ID =? ??? =? ??????
    mapping (uint256 => mapping (bytes32 => bool)) public plyrNames_; // ??ID =? ????? =? True/False

    mapping (uint256 => F3Ddatasets.Round) public round_;   // ??? =? ????
    mapping (uint256 => mapping(uint256 => uint256)) public rndTmEth_;      // ???=?TeamID=>????

    mapping (uint256 => F3Ddatasets.TeamFee) public fees_;          // TeamID => ????????
    mapping (uint256 => F3Ddatasets.PotSplit) public potSplit_;     // TeamID => ????????
