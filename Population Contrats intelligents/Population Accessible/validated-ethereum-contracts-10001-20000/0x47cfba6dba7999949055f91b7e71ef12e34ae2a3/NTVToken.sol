...
contract NTVUToken is BasicToken, Ownable, Auction {
    ...
    uint8 public auditStatus = 0; // 0:????1:?????2:?????
     ...
     *
     * @param _number ????????0??
     * @param _bidStartValue ?????? wei
     * @param _bidStartTime ??/?????????s
     * @param _bidEndTime ??/?????????s
     * @param _tvUseStartTime ???????????
     * @param _tvUseEndTime ???????????
     * @param _isPrivate ?????
     * @param _defaultText ????
     * @param _ethSaver ???????
    ...
    function bid() public payable returns (bool) {
        ...
        require(msg.value >= maxBidValue + 0.05 ether); // ??0.05ETH??
    ...
    function setText(string _text) public {
        ...
        require(bytes(_text).length > 0 && bytes(_text).length <= 90); // ????UTF8???1???????3?????????90?????
        require(now < tvUseStartTime - 30 minutes); // ???30?????????
    ...
    function auditText(uint8 _status, string _text) external onlyOwner {
        require((now >= tvUseStartTime - 30 minutes) && (now < tvUseEndTime)); // ?????30???????????????????
        auditStatus = _status;
    ...
    function reclaimEther() external onlyOwner {
        require((now > bidEndTime) || (isPrivate && maxBidAccount != address(0))); // ??????????????????????ethSaver?
        ethSaver.transfer(this.balance);
    }
...
contract NTVToken is Ownable {
    ...
    mapping(uint => address) internal timeRanges; // ?????????????0??
     ...
     * ???????
     *
     * @param _onlineTime ?????????????????? 2018-03-26 00:00:00
     * @param _ethSaver ????ETH???
     */
    function startup(uint256 _onlineTime, address _ethSaver) public onlyOwner {
        require(!isRunning); // ??????????????
        require((_onlineTime - 57600) % 1 days == 0); // ????????????57600??????'1970/1/2 0:0:0'
        ...
        for (uint8 i=0; i<6; i++) {
            dayConfigs[i].bidStartValue = 0.1 ether; // ?????0.1ETH
            dayConfigs[i].bidStartTime = 18 hours + 30 minutes - 1 days; // ????? 18:30??
            dayConfigs[i].bidEndTime = 22 hours - 1 days; // ????? 22:00 ????
        ...
        for(uint8 p=0; p<6; p++) {
            ...
            specialConfigs[p].bidStartValue = 0.1 ether; // ???0.1ETH
            specialConfigs[p].bidStartTime = 18 hours + 30 minutes - 2 days; // ????18:30
            specialConfigs[p].bidEndTime = 22 hours - 1 days; // ????22:00
