...
contract CryptoQuantumTradingFund is ERC20Interface {
	...
	event Transfer(address indexed from, address indexed to, uint tokens);//transfer??????????
	event Approval(address indexed tokenOwner, address indexed spender, uint tokens); //approve??????????
