...
/*
	...
	Liftershandboek), Sweden (Liftarens guide till galaxen), Finland (Linnunradan Käsikirja Liftareille) and also Israel (????? ???????? ???????).  In
	the meantime the book has been translated into more than thirty  languages,  such  as  Bulgarian  (??????????? ?? ????????????? ?????????),  Czech
	(Stopa??v pr?vodce Galaxií), Farsi/Persian (??????? ??????? ????? ??????), French (Le routard galactique), Greek (??????? ?? ??????? ?? ???-????),
	Hungarian (Galaxis Útikalauz stopposoknak), Italian (Guida galattica per gli autostoppisti),
	Japanese (????????????), Korean (???? ?????????? ?? ???),
	Latvian  (Galaktikas   ce?vedis   stop?t?jiem),   Norwegian   (Haikerens   guide   til   Galaksen,   first   published   as  På  tommeltotten  til
	melkeveien), Brazilian Portuguese (Guia do Mochileiro das Galáxias), Portuguese (À Boleia Pela  Galáxia),  Polish  (Autostopem  przez  galaktyk?),
	Romanian (Ghidul autostopistului galactic), Russian (?????????? ?? ?????????), Serbian (Autostoperski vodi? kroz galaksiju), Slovenian  (Štoparski
	vodnik po Galaksiji), Spanish (Guía del autoestopista galáctico), Slovak (Stopárov sprievodca galaxiou), Czech  (Stopa??v  pr?vodce  galaxií)  and
