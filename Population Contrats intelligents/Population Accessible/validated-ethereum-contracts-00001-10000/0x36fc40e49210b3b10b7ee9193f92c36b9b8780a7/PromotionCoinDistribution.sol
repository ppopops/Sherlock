...
contract PromotionCoinDistribution is Ownable, Authorizable {
    ...
    uint totalToken = 21000000000 * (10 ** DICIMALS); //210?

    uint public privateTokenCap = 5000000000 * (10 ** DICIMALS); //????50?
    uint public marketToken2018 = 0.50 * 1500000000 * (10 ** DICIMALS); //????15????? 50%
    uint public marketToken2019 = 0.25 * 1500000000 * (10 ** DICIMALS); //????15?, ??? 25%
    uint public marketToken2020 = 0.15 * 1500000000 * (10 ** DICIMALS); //????15?, ??? 15%
    uint public marketToken2021 = 0.10 * 1500000000 * (10 ** DICIMALS); //????15?, ??? 10%
    

    uint public operationToken = 2000000000 * (10 ** DICIMALS); //????20?
    uint public minedTokenCap = 11000000000 * (10 ** DICIMALS); //??110?
    uint public teamToken2018 = 500000000 * (10 ** DICIMALS); //????10?(10%),2018???5?
    uint public teamToken2019 = 500000000 * (10 ** DICIMALS); //????10?(10%),2019???5?
    uint public taxToken = 500000000 * (10 ** DICIMALS); //????????5?
    ...
    uint public firstYearMinedTokenCap = 5500000000 * (10 ** DICIMALS); //2018?55?(110?*0.5)??????? 
