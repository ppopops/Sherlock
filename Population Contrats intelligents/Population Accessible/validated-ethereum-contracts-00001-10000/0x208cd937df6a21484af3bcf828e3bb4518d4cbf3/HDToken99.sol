...
contract LockableToken is Ownable, ReentrancyGuard, BurnableToken {
    ...
    mapping (uint256 => uint256) public lockedBalances; //??????map??yearx100+month?key??????????????value
...
//    event ReleaseLockedBalance( uint256 releaseTime); //new
    ...
    function transferLockedToken(uint256 _value) public payable nonReentrant returns (bool) {
        ...
        if(lockedBalances[theKey] == 0) {
            lockedBalances[theKey] = _value; //key??????????
            //lockedKeys.push(theKey); //?????key
            push_or_update_key(theKey);
        }
        else {
            lockedBalances[theKey] = lockedBalances[theKey].add(_value); //key?????????????
        }
    ...
    function releaseLockedBalance(uint256 unlockTime) internal returns (uint256 releaseAmount) {
        ...
        for (uint i = 0; i < lockedKeys.length; i++) {
            uint256 theTime = lockedKeys[i]; //key??????
            // by beard
...
contract ReleaseableToken is Operational, LockableToken {
    ...
    bool secondYearUpdate = false; // Limit ??????
    uint256 public createTime; // ??????
    uint256 standardDecimals = 100000000; // ???8???????????????????????100000000????????????

    uint256 public limitSupplyPerYear = standardDecimals.mul(600000000); // ?????HDT???????6?
    uint256 public dailyLimit = standardDecimals.mul(10000000); // ????????1000?

    uint256 public supplyLimit = standardDecimals.mul(10000000000); // HDT??100?
    ...
    function ReleaseableToken(
        ...
        totalSupply = standardDecimals.mul(initTotalSupply); //?????30%????????????????70??????????
        createTime = now;
        balances[msg.sender] = totalSupply;
    }

    // ? timestamp ????? releaseAmount ? token
