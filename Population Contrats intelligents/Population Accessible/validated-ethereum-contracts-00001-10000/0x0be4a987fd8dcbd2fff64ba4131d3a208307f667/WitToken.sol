...
//@dev ??????openzeppelin?erc20??
//1.??openzeppelin?SafeMath???????
//2.??openzeppelin?Ownable,Roles,RBAC??????,????ceo,coo,cro???  
//3.ERC20???ERC20Basic????????
//4.BasicToken,StandardToken,PausableToken??erc20?????
//5.BlackListToken???????
//6.TwoPhaseToken?????????,?????????????
//7.UpgradedStandardToken???TetherUSD??,??????????erc20???
//8.???????????
...
/**
* ?????token????????????
* 1.?????????submitIssue?submitRedeem;
* 2.?????????comfirmIsses?comfirmRedeem;
* 3.????????????????????
* 4.?????????????????????????????????
**/
...
contract WitToken is TwoPhaseToken {
   ...
   * @param newBasisPoints  A few ten-thousandth (???????????)
   * @param newMaxFee Maximum fee (???????,?????decimals)
   */
